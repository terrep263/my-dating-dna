...
interface GraceConfig {
  ...
  callbacks: {
    ...
    onMessage: ((message: { role: string; content: string; type?: string }) => void) | null;
  };
}

interface GraceAIProps {
  config?: Partial<GraceConfig>;
}
...
const DEFAULT_CONFIG = {
  ...
  texts: {
    ...
    description:
      "Discover your authentic relationship blueprint through evidence-based insights and transformative questioning. Available 24/7 to guide your journey to lasting love.",
...
class GraceResponseEngine {
  private harmfulPatterns: string[];
  ...
  async analyzeMessage(message: string, conversationHistory: { role: string; content: string }[] = []) {
    const lowerMessage = message.toLowerCase();

    // Check for harmful content first
    const isHarmful = this.harmfulPatterns.some((pattern) =>
    ...
    try {
      if (
        ...
        console.warn(
          "OpenAI API key is missing or invalid, using local patterns"
        );
      }
    } catch (error) {
  ...
  async getOpenAIResponse(message: string, conversationHistory: { role: string; content: string }[] = []) {
...
Respond as Grace would - warmly, intelligently, and with practical wisdom. Keep responses under 150 words.`;
  ...
  getLocalResponse(message: string) {
    const lowerMessage = message.toLowerCase();
    ...
    if (
      ...
      lowerMessage.includes("extrovert")
    ) {
      const responses = [
...
const GraceAI: React.FC<
  GraceAIProps & {
    onMessage?: ((message: { role: string; content: string; type?: string }, allMessages: { role: string; content: string; type?: string }[]) => void) | null;
    onChatStart?: (() => void) | null;
    onEmergencyExit?: (() => void) | null;
    className?: string;
...
> = ({
  ...
  const [messages, setMessages] = useState([
    {
      role: "grace",
      content:
        "Hello, I'm Grace - your AI relationship coaching assistant. I'm designed to help you explore authentic self-discovery and sustainable love patterns through evidence-based insights and transformative questioning. While I'm not a human therapist, I can guide you through relationship exploration using proven psychological frameworks. What would you like to explore today?",
    },
  ]);
  ...
  }, [mergedConfig.features.emergencyExit, handleEmergencyExit]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const analysis = await responseEngine.analyzeMessage(
        currentInput,
        conversationHistory
      );

      if ((analysis as any)?.showCrisisResources) {
        setShowResources(true);
      }

      const graceResponse = {
        role: "grace",
        content: (analysis as any)?.response,
        type: (analysis as any)?.type,
      };
      ...
      const fallbackResponse = {
        role: "grace",
        content:
          "I'm having trouble connecting right now. Let me try to help with what I can. Could you rephrase your question or try again in a moment?",
        type: "fallback",
      };
          ...
          <div className="p-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center mb-3">
                      <IconComponent
                        className={`w-5 h-5 text-${category.color}-600 mr-2`}
                      />
                      <h4
                        className={`font-semibold text-${category.color}-800`}
                      >
                        {category.category}
                      </h4>
                    </div>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-lg text-center">
              <p className="text-sm text-gray-600">
                <strong>Remember:</strong> You deserve support, safety, and
                professional care. These resources are staffed by trained
                professionals who can provide immediate assistance.
              </p>
              <p className="text-xs text-gray-500 mt-2">
                Grace AI provides educational support but is not a substitute
                for professional mental health treatment or human counseling.
              </p>
            </div>
          </div>
        </div>
  ...
  if (showChat) {
    ...
    return (
      <div
        className={`min-h-screen ${mergedConfig.theme.background} ${className}`}
      >
        <EmergencyExitButton />
        ...
        <div className="backdrop-blur-sm bg-white/10 border-b border-white/20 p-6">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                <div
                  className={`w-10 h-10 bg-gradient-to-r ${mergedConfig.theme.primary} rounded-full flex items-center justify-center mr-4 shadow-lg`}
                >
                  <Heart className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h3 className="font-semibold text-white text-lg">
                    {mergedConfig.texts.brandName}
                  </h3>
                  <p className="text-sm text-white/70">
                    AI Relationship Coaching Session
                  </p>
                </div>
              </div>
            </div>
                    ...
                    >
                      {message.role === "grace" && (
                        <div className="flex items-center mb-3 pb-3 border-b border-white/30">
                          <div
                            className={`w-8 h-8 bg-gradient-to-r ${mergedConfig.theme.primary} rounded-full flex items-center justify-center mr-3 shadow-lg`}
                          >
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-white">
                          ...
                          <span className="text-xs text-white/70 block mt-1">
                            {(responseEngine as any).openaiApiKey &&
                            (responseEngine as any).openaiApiKey.trim() &&
                            (responseEngine as any).openaiApiKey.startsWith("sk-")
                              ? `Using AI intelligence (${(responseEngine as any).openaiModel})`
            ...
            <div className="border-t border-white/20 backdrop-blur-sm bg-white/10 p-6">
              <div className="flex space-x-4">
                <TextField
                  ...
                  className="!text-white flex-1 p-4 backdrop-blur-sm bg-white/20 border border-white/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent resize-none text-sm  placeholder-white/80"
                  rows="3"
                />
                <button
                  onClick={sendMessage}
  ...
  // Full Landing Page
  console.log(
    "Rendering Full Landing Page - showLandingPage is true and showChat is false"
  );
  return (
    <div
    ...
    >
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <p className="text-xl text-white/90 mb-6 leading-relaxed">
                I'm here to help you navigate the beautiful complexity of love
                and relationships. As your personal relationship coach, I'm
                available 24/7 to provide professional guidance and insights
                based on real relationship science.
              </p>
              <p className="text-lg text-white/80 mb-10 leading-relaxed">
                Whether you're wondering about that text you just received,
                preparing for a difficult conversation, or trying to understand
                your dating patterns, I'm here to help you find clarity and
            ...
            <div className="lg:text-right">
              <div className="text-center">
                <div className="relative group">
                  ...
                  <div
                    className="absolute top-12 left-6 w-1 h-1 bg-purple-300 rounded-full opacity-60 animate-pulse"
                    style={{ animationDelay: "1s" }}
                  ></div>
                  <div
                    className="absolute bottom-16 left-4 w-1.5 h-1.5 bg-blue-300 rounded-full opacity-70 animate-ping"
                ...
                <div className="flex items-center justify-center mt-6 mb-4">
                  ...
                  <span className="text-white text-lg font-bold tracking-wide animate-pulse">
                    ONLINE NOW
                  </span>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600">
              Your personal AI coach with a passion for helping hearts connect
            </p>
          </div>
            ...
            <div className="prose prose-lg text-gray-700 leading-relaxed">
              <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-8 rounded-2xl mb-8 border border-purple-100">
                ...
                <p className="mb-4 text-lg">
                  Hi there! I'm Grace, and I'm genuinely excited to be part of
                  your relationship journey. I was created with one clear
                  purpose: to be your dedicated relationship coach - someone who
                  provides professional, unbiased guidance whenever love feels
                  complicated.
                </p>
                <p className="mb-4">
                  What makes me different? I don't bring my own relationship
                  baggage to our conversations (because I don't have any!). I
                  won't project my experiences onto your situation or nudge you
                  toward what worked for someone else. Instead, I listen to YOUR
                  story, understand YOUR values, and help you find solutions
                  that feel authentically right for you.
                </p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8 mb-8">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <p className="text-gray-700">
                    Think of our conversations like working with a skilled
                    relationship coach who's deeply knowledgeable about love and
                    dating. You share what's on your mind, I ask thoughtful
                    coaching questions to help you see things clearly, and
                    together we develop your action plan.
                  </p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  <h4 className="text-xl font-semibold text-gray-900 mb-3 flex items-center">
                  ...
                  <p className="text-gray-700">
                    I believe every person and relationship is unique. That's
                    why I never give cookie-cutter advice. Instead, I help you
                    discover what works for YOUR personality, YOUR situation,
                    and YOUR goals in love.
                  </p>
                </div>
              </div>
              ...
              <p className="mb-6 text-lg bg-emerald-50 p-6 rounded-xl border border-emerald-200">
                ...
                I'm here whenever you need me - whether it's 2 AM and you're
                overthinking a text, or you're preparing for a big relationship
                conversation. I'll help you sort through your thoughts,
                understand your patterns, and move forward with confidence. No
                judgment, no agenda - just genuine support for your love life.
              </p>

              <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mt-8">
                <p className="text-sm text-yellow-800 font-medium">
                  <strong>Disclaimer:</strong> Grace AI is not a tool for crisis
                  intervention. If you are in immediate danger or facing a
          ...
          <div className="grid md:grid-cols-4 gap-8">
            <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600">
                "Whether it's 3 AM anxiety or a lunch break question, I'm ready
                to chat whenever you need me."
              </p>
            </div>
            <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600">
                "I've learned from thousands of relationship stories, so I can
                help you see patterns and possibilities you might miss."
              </p>
            </div>
            <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600">
                "Share anything with me - I don't judge, I don't gossip, and
            ...
            <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600">
                "I get to know your unique style and preferences, so my advice
      ...
      {/* CTA Section */}
      <section
        className={`py-20 bg-gradient-to-r ${mergedConfig.theme.primary}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-purple-100 mb-4">
            I'm excited to meet you and learn about your relationship journey.
          </p>
          <p className="text-lg text-purple-200 mb-8">
            Our first conversation is completely free - no strings attached,
          ...
          <p className="text-sm text-purple-200 mt-4 italic">
            "I can't wait to help you find clarity and confidence in love!"
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="flex items-center justify-center mb-6">
              <div
                className={`w-8 h-8 bg-gradient-to-r ${mergedConfig.theme.primary} rounded-lg flex items-center justify-center mr-3`}
              >
                <Heart className="w-4 h-4 text-white" />
              </div>
            ...
            <p className="text-gray-300 mb-6 max-w-2xl mx-auto">
              Hi! I'm Grace, your AI relationship coach. While I love helping
              with dating and relationship questions, I'm not a replacement for
              professional therapy or medical care. For serious mental health
              concerns, please reach out to qualified human professionals.
            </p>
